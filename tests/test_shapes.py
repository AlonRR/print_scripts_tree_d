...
def test_make_hexagonal_mesh_has_cutouts() -> None:
    panel = make_hexagonal_mesh(length=30, width=30, thickness=3, hex_radius=2, spacing=1)
    solid_volume = 30 * 30 * 3
    assert panel.volume < solid_volume


def test_make_hexagonal_mesh_with_border() -> None:
    # Border should add material back along the perimeter, so volume with border > without.
    without = make_hexagonal_mesh(length=30, width=30, thickness=3, hex_radius=2, spacing=1)
    with_border = make_hexagonal_mesh(
        length=30, width=30, thickness=3, hex_radius=2, spacing=1, outer_border=3
    )
...
def test_make_column_gusset_warning(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.WARNING):
        make_column(
            body=body, height=40, foot=foot, diameter=None, gusset_size=5, gusset_thickness=0
        )
